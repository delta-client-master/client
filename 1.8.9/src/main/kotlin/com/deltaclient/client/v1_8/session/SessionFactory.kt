package com.deltaclient.client.v1_8.session
...
object SessionFactory : ISessionFactory {
    ...
    override fun createMojangSession(email: String, password: String): ISessionBridge {
        ...
        if (userAuth.isLoggedIn) {
            ...
            return Session(
                profile.name, profile.id.toString(), userAuth.authenticatedToken, Session.AccountType.MOJANG.name
            ).cast()
        }
    ...
    override fun createMicrosoftSession(username: String, uuid: String, token: String): ISessionBridge {
        return Session(username, uuid, token, Session.AccountType.MOJANG.name).cast()
    }
}
